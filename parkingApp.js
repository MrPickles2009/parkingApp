...
//console.log("Mon-Thu, Fri,Sat-Sun" + red + "24 hrs/day")

let status = prompt("What is your status at JMU? (Resident, Commuter, Visitor, Staff)");
if (status == "Resident") {
	console.log("You can park in the following zones: " + resident);
} else if (status == "Commuter") {
	console.log("You can park in the following zones: " + commuter);
} else if (status == "Visitor") {
	console.log(visitor);
} else if (status == "Staff") {
	var staff = prompt("Are you Red Zone or Blue Zone?");
	if (staff == "Red Zone") {
